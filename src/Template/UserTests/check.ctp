                ...
                <div class="d-none d-md-block col-md-2 question-numbering">
                    <ul class="nav nav-tabs nav-stacked">
                        <?php foreach($initial_result['questions'] as $index=>$question):?>
                            <li class=" nav-li<?= ($index == 0) ? ' active' : '' ?> <?= $question->status ?>" question_id="<?= $question->id?>">
                                <?= 
                                    $this->Html->link(
                                        'Question #'.($index+1),
                            ...
                            <div class="tab-pane<?= ($index == 0) ? ' active' : ''; ?>" id="<?= 'question_div-'.$question->id ?>" question-number="<?= $index+1 ?>">
                                ...
                                <?php if($question->course_question_type_id == 1) :?>
                                    <?php foreach( $question->course_question_choices as $index=>$choice ):?>
                                        <div class="choices-container <?= (isset($question->user_answers[0]) && $question->user_answers[0]->answer_id == $choice->id) ? 'user-answer' : '' ?> <?= (isset($question->user_answers[0]) && $question->answers[0]->id == $choice->id) ? 'correct-answer' : '' ?>">
                                            <p><?= h($choice->value); ?></p>
                                        </div>
                                    <?php endforeach;?>
